...
from Efficient_modules.graphs import Graph
from Efficient_modules.layers import Basic_Layer, Spatial_Graph_Layer
from Efficient_modules.activations import Swish
from Efficient_modules.attentions import ST_Joint_Att
import numpy as np
...
class impSGN(nn.Module):
    def __init__(self, num_classes, dataset, seg, args, bias=True):
        ...
        ### changed for state04
        self.tem_embed = embed(self.seg, self.dim1 * 2, norm=False,
                               bias=bias)  # self.tem_embed = embed(self.seg, 64*4, norm=False, bias=bias)
        ...
        self.cnn = local(self.dim1 * 2, self.dim1 * 2,
                         bias=bias)  # self.cnn = local(self.dim1, self.dim1 * 2, bias=bias)
    ...
    ### added:
    #        self.bn_relu = norm_act(self.dim1)
    #        self.output_feature = 42*6
    #        self.mstcn = MultiScale_TemporalConv(self.dim1, self.output_feature) #42*6=252   #the output_features must be divided by 6 {6: 4 Branches + 2}
    #        self.upsampling = nn.Conv2d(in_channels=self.output_feature, out_channels=self.dim1, kernel_size=1, bias=bias)
    #        self.residual = nn.Conv2d(in_channels=self.dim1//2, out_channels=self.dim1, kernel_size=1, bias=bias)
...
if __name__ == '__main__':
    ...
    args.num_classes = 60
    model = impSGN(args.num_classes, args.dataset, args.seg, args)
